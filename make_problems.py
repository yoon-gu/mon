...
MULT_TEMPLATE = "\item  \opmul[resultstyle=\placeholder,voperation=top,voperator=bottom,intermediarystyle=\placeholder]{{{input1}}}{{{input2}}}"
...
def main(num_problems, lower, upper, ptype):
    ...
    with open('problems_list.tex', 'w') as f:
        f.write('\n'.join([p.replace(",intermediarystyle=\placeholder", "").replace("resultstyle=\placeholder,", "").replace("carryadd=false","carryadd=true") for p in problems]))
